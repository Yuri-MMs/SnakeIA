...
snake_block = 10
snake_speed = 5  # Velocidade da cobra
...
def gameLoop():
    ...
    while waiting:
        ...
        message = font_style.render("Pressione SPACE para começar", True, WHITE)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    waiting = False  # Sai do loop de espera e inicia o jogo
    ...
    # Variáveis para suavizar o movimento do rosto
    smoothing_factor = 0.2  # Controle de suavização, ajustado para ser mais leve
    ...
    while not game_over:
        ...
        # Suavizar os movimentos do rosto para evitar movimentos bruscos
        x_center = int(last_x_center * (1 - smoothing_factor) + x_center * smoothing_factor)
        y_center = int(last_y_center * (1 - smoothing_factor) + y_center * smoothing_factor)
        ...
        if abs(x_center - last_x_center) > move_threshold or abs(y_center - last_y_center) > move_threshold:
            ...
            if x_center < WIDTH // 3:  # Lado esquerdo da tela
                if x_change == 0:  # Só muda para a esquerda se não estiver indo para a direita
                    x_change = -snake_block
                    y_change = 0
            elif x_center > 2 * WIDTH // 3:  # Lado direito da tela
                if x_change == 0:  # Só muda para a direita se não estiver indo para a esquerda
                    x_change = snake_block
                    y_change = 0
            elif y_center < HEIGHT // 3:  # Parte superior da tela
                if y_change == 0:  # Só muda para cima se não estiver indo para baixo
                    x_change = 0
                    y_change = -snake_block
            elif y_center > 2 * HEIGHT // 3:  # Parte inferior da tela
                if y_change == 0:  # Só muda para baixo se não estiver indo para cima
                    x_change = 0
                    y_change = snake_block
